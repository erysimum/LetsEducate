...
class Root extends React.Component {
  componentDidMount() {
    console.log('START', this.props.isLoading);
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        console.log('user', user);
        this.props.setUser(user); //action
  ...
  render() {
    return this.props.isLoading ? (
      <Spinner spinnerLabel='Preparing Dashborad...' />
    ) : (
      <Switch>
...
const RootWithAuth = withRouter(connect(mapStateToProps, { setUser, clearUser, clearVideos })(Root));
