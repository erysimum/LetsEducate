...
const getResults = (videos) => {
  console.log('videoxxxxxxxxxxs', videos);
  const results = {};
  const topics = getTopics(videos);
  for (let topic of topics) {
    results[topic] = {
      name: topic,
      results: Object.values(videos)
        .filter((video) => video.videoTopic.toLowerCase() === topic)
        .map((video) => ({
...
const getTopics = (videos) => {
  console.log('getTopics in Search...', videos);
  const topics = [];
...
class SearchExampleCategory extends Component {
  ...
  handleResultSelect = (e, { result }) => {
    ...
    console.log('result.....', result); //video Obj
    // this.setState({ value: result.title })
    this.props.history.push(`/video/${result.id}`);
  };

  handleSearchChange = (e, { value }) => {
    console.log('HandleSearchChange.....triggers 1st');
