...
class PlayVideo extends React.Component {
  ...
  componentDidMount() {
    ...
    if (videos && currentVideoId) {
      ...
      console.log('currentVideo in PlayVideo ....', this.getCurrentVideo(videos, currentVideoId)[0]);
    }
    this.checkFavourite(videos, currentVideoId);
    console.log('this.checkFav ....', this.state.isAFavourite);
  }
  ...
  checkFavourite = (videos, currentVideoId) => {
    if (this.state.user.favourites.includes(videos[currentVideoId])) this.setState({ isAFavourite: true });
  };
  ...
  addToFavorite = () => {
    ...
    usersRef
      ...
      .set({
        ...
        avatar,
        role,
        favourites: isAFavourite ? favourites.filter((item) => item !== currentVideo.id) : [...favourites, currentVideo.id]
      })
      .then(() => {
        this.setState({
  ...
  render() {
    ...
    return Object.keys(currentVideo).length === 0 ? (
      <Spinner spinnerLabel='Loading Video...' />
    ) : (
      <React.Fragment>
        <Segment basic>
          <Grid>
