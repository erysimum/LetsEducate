...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    console.log('Hello 2222222 from this.render in App');
    ...
    if (!user.role) {
      firebase
        ...
        .then((snap) => snap.val())
        .then((val) => {
          //val is object of user from database {avatar, name,role}
    ...
    if (videos.length === 0) {
      videosRef
        ...
        .then((val) => {
          ...
          this.props.setVideos(val); // add video to state via action
        });
    }
  }
  ...
  render() {
    ...
    console.log('Hello 111111111I from this.render in App');

    console.log('showFavourites', showFavourites);
    console.log('videosss in APp', videos);
    return (
      <React.Fragment>
              ...
              <Grid.Row columns={3}>
                {videos &&
                  Object.values(videos)
                    .filter((video) => user.favourites.includes(video.id))
                    .slice(0, 3)
                    .map((video, i) => (
                      <Grid.Column key={video.id}>
                        ...
                        <Card.Content>
                          <Card.Header>
                            <Header>{video.videoTitle}</Header>
                          </Card.Header>
                          <Card.Meta>
        ...
        <Segment vertical>
          ...
          <Grid>
            <Grid.Row columns={3}>
              {videos &&
                Object.values(videos)
                  .sort((a, b) => a.uploadedOn - b.uploadedOn)
                  .slice(0, 3)
                  .map((video, i) => (
                    <Grid.Column key={video.id}>
                      <Link to={`/video/${video.id}`}>
        ...
        <Segment inverted vertical>
          <Header as='h5' textAlign='right'>
            ©LetsEducate, 2021. All rights reserved.
          </Header>
        </Segment>
      </React.Fragment>
    );
